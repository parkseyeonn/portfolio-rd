$(document).ready(function(){
	var page_wrap = $(".layout");
	var page_name = $(".fullpage");
	var page_inner_name = $(".cts_wrapper");
	var page_css = "on";
	var window_h = $(window).height();
	var window_w = $(window).width();
	var index_wrap_name = $("aside");
	var index_name = $("aside ul li");
	var index_length = index_name.length;
	var index_css = "on";
	var mobile_size = 1100;/* 모바일로 변환되는 사이즈 (자동스크롤, 자동높이지정 해제) */
	...
	var prev_index;
	var page_total;
	var navigation = $('.visual .navigation ul li');
	var scroll_flag;
	...
	function page_scroll(scroll,scrollParent){
		if(scrollParent == 'navigation'){
			scroll_flag = 2;
		} else {
			scroll_flag = 1;
		}
		if(scroll.index()+scroll_flag != page_index){
			prev_index = page_index;
			page_index = scroll.index()+scroll_flag;
