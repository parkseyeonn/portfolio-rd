...
/*
	팝업 이미지가 모두 로딩된 후에 높이를 계산해야 하므로 $(window).load로 한다.
*/

	var window_w = $(window).width();
	var obj_wrap = $(".sns .banner"); //높이 제어
	var obj_name = $(".sns .banner ul"); // 실제 움직일놈
	var obj_child = $(".sns .banner ul li");// 하나의 팝업

	//팝업 컨트롤버튼
	var ctrl_btn = true;//사용여부 (true, false)
	var ctrl_next = $(".sns .ctrl_btn .next");
	var ctrl_prev = $(".sns .ctrl_btn .prev");
	var ctrl_stop = $(".sns .ctrl_btn .stop");
	var ctrl_play = $(".sns .ctrl_btn .play");
	...
	if(1500 <= window_w && window_w < 2200){
		pc_view = 5;
	} else if(1200 <= window_w && window_w < 1500){
		pc_view = 4;
	} else if(800 <= window_w && window_w < 1200){
		pc_view = 3;
	} else if(640 < window_w && window_w < 800){
		pc_view = 2;
	} else{
		pc_view = 6;
	}
	...
	function view_count(){
		...
		obj_wrap.height(obj_child.height()); // 팝업의 높이값도 제어 (모바일에서 넓이가 줄면 높이도 줄이기 위해서)
	...
	function auto_next(){
		...
		if(obj_index > obj_length){ //더이상 복제한 팝업이 없을때 
			obj_index = 1;
